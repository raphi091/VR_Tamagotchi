...
//			GUILayout.EndHorizontal();
	...
	public void OnSceneGUI() {
		...
		// control_points
		for(i=0; i<_target.control_points.Count; i++) {
			var fmh_800_28_638878435756624237 = Quaternion.identity; Vector3 vec= 
				Handles.FreeMoveHandle(T_lw(_target.control_points[i]), HandleUtility.GetHandleSize(T_lw(_target.control_points[i]))*0.08f, Vector3.one, 
		...
		if ((_target.state==0) || (_target.show_tesselation_points) && (!_target.paint_height)) {
			for(i=0; i<_target.tesselation_points.Count; i++) {
				var fmh_823_29_638878435756650003 = Quaternion.identity; Vector3 vec= 
					Handles.FreeMoveHandle(T_lw(_target.tesselation_points[i]), HandleUtility.GetHandleSize(T_lw(_target.tesselation_points[i]))*0.06f, Vector3.one, 
		...
		if (_target.GetComponent<Renderer>() && _target.GetComponent<Renderer>().sharedMaterial && _target.GetComponent<Renderer>().sharedMaterial.HasProperty("UVbounds")) {
			...
			var fmh_843_43_638878435756652938 = Quaternion.identity; vec=Handles.FreeMoveHandle(handle_pos, HandleUtility.GetHandleSize(handle_pos)*0.04f, Vector3.one, Handles.DotHandleCap);
			...
			var fmh_855_43_638878435756654802 = Quaternion.identity; vec=Handles.FreeMoveHandle(handle_pos, HandleUtility.GetHandleSize(handle_pos)*0.04f, Vector3.one, Handles.DotHandleCap);
			if (Mathf.Abs(vec.x-handle_pos.x)>0) {
				if (!_target.undo_flag) {
					Undo.RecordObject(_target, "grass edit");
					_target.undo_flag=true;
			...
			var fmh_867_43_638878435756656514 = Quaternion.identity; vec=Handles.FreeMoveHandle(handle_pos, HandleUtility.GetHandleSize(handle_pos)*0.04f, Vector3.one, Handles.DotHandleCap);
			...
			var fmh_879_43_638878435756657998 = Quaternion.identity; vec=Handles.FreeMoveHandle(handle_pos, HandleUtility.GetHandleSize(handle_pos)*0.04f, Vector3.one, Handles.DotHandleCap);
			if (Mathf.Abs(vec.z-handle_pos.z)>0) {
				if (!_target.undo_flag) {
					Undo.RecordObject(_target, "grass edit");
					_target.undo_flag=true;
			...
			if (_target.which_active!=3) {
				...
				for(i=0; i<_target.control_points.Count; i++) {
					if ( (_target.active_idx==i) && ((_target.which_active==1) || (Vector3.Distance(T_lw(_target.control_points[i]), T_lw(_target.bezier_pointsA[i]))>0.01f)) ) {
						var fmh_898_31_638878435756660785 = Quaternion.identity; Vector3 vec= 
							Handles.FreeMoveHandle(T_lw(_target.bezier_pointsA[i]), HandleUtility.GetHandleSize(T_lw(_target.bezier_pointsA[i]))*0.05f, Vector3.one, 
							                       Handles.CircleHandleCap);
				...
				for(i=0; i<_target.control_points.Count; i++) {
					if ( (_target.active_idx==i) && ((_target.which_active==2) || (Vector3.Distance(T_lw(_target.control_points[i]), T_lw(_target.bezier_pointsB[i]))>0.01f)) ) {
						var fmh_918_31_638878435756663405 = Quaternion.identity; Vector3 vec= 
							Handles.FreeMoveHandle(T_lw(_target.bezier_pointsB[i]), HandleUtility.GetHandleSize(T_lw(_target.bezier_pointsB[i]))*0.05f, Vector3.one, 
							                       Handles.CircleHandleCap);
